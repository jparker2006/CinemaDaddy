...
A small chatbot that answers questions about movies and TV shows by driving Claude's tool-use loop against TMDB and OMDb. Streams responses token-by-token, renders posters and markdown inline, surfaces tool calls as breadcrumbs, and suggests follow-up questions after every reply. Built as a practice project for the Anthropic tool-use pattern. Stateless serverless function on Vercel; conversation history lives in the browser's localStorage.
...
```bash
npm install
npm i -g vercel              # one-time CLI install (or use npx vercel)
vercel link                  # link this folder to your Vercel project
vercel env pull .env.local   # pull the env vars you configured in the Vercel dashboard
npm run dev                  # starts vercel dev — serves static frontend + api/chat function
```

Open the URL `vercel dev` prints (defaults to [http://localhost:3000](http://localhost:3000)).

### Deploy to Vercel

Push to a branch tracked by Vercel — the deploy is automatic. Build command: `npm run build`. Output directory: `dist/` (auto-detected via the Vite framework preset in `vite.config.ts`). The serverless function at `api/chat.ts` is detected by convention. Required env vars in the Vercel dashboard: `ANTHROPIC_API_KEY`, `TMDB_BEARER_TOKEN`, `OMDB_API_KEY`, optionally `STREAMING_REGION` (default `US`).
...
```
api/
└── chat.ts                  # Vercel Node serverless function. POST handler with
                             # ReadableStream → NDJSON stream of ChatEvents.
                             # Stateless: client sends full conversation each turn.

src/                         # Shared backend modules (imported by api/chat.ts)
├── chat.ts                  # Tool-use loop + Haiku follow-ups (runTurn)
├── anthropic.ts             # SDK client + MODEL (Sonnet) + HAIKU_MODEL
├── systemPrompt.ts          # Behavior rules incl. posters, trailers, mood mode
├── clients/
...
frontend/                    # React + Vite source (built to dist/ by `npm run build`)
├── index.html
├── tsconfig.json
└── src/
    ├── main.tsx             # React entry
    ├── App.tsx              # Chat UI: canonical MessageParam[] in localStorage,
                             # NDJSON stream consumer, derived render
    ├── styles.css           # Dark theater theme
    └── vite-env.d.ts

vite.config.ts               # root: frontend/, outDir: ../dist (Vercel-detected)
```

`dist/` contains the built React bundle and is regenerated by every `npm run build`; it's `.gitignore`d.

Full design notes:
- v1 (initial implementation): [`PLAN.md`](./PLAN.md)
- v2 (posters, cast & crew tool, follow-up chips): [`PLAN-v2.md`](./PLAN-v2.md)
- Mood mode (vibe-based discovery): [`PLAN-mood.md`](./PLAN-mood.md)
- Vercel migration (stateless function + client-side conversation): [`PLAN-vercel.md`](./PLAN-vercel.md)

## Scripts

- `npm run dev` — `vercel dev`: runs the static frontend + `api/chat` serverless function locally on the Vercel emulator. Requires `vercel link` once. Default port `:3000`.
- `npm run build` — `vite build`: produces the production bundle in `dist/` (consumed by Vercel for the static site).
- `npm run typecheck` — strict TS check for both `src/` + `api/` (Node) and `frontend/` (React).
...
- **Stateless server, client-side conversation**: the serverless function has no memory between invocations. The browser holds the canonical `Anthropic.MessageParam[]` in `localStorage` under the key `cinema-daddy-conversation` and posts it up with every turn. Refresh keeps your chat; the **+ New Chat** button clears the key. A 4 MB soft cap trims oldest turns automatically when the conversation gets large.
- **Streaming**: `api/chat.ts` returns a `ReadableStream` body (NDJSON, one event per line). The Vercel Node runtime + Fluid Compute hold the connection open up to 60s, which covers even tool-heavy mood queries.
...
- **No database, no auth**: this is a hobby/portfolio project. Anyone hitting `/api/chat` on the deployed URL will burn your Anthropic credits. Consider Vercel's rate limiting / authentication features before sharing the deploy URL publicly.
